...
export default function PlumFlowerPage() {
  ...
  return (
    <DivinationCardHeader
      title={t('divination.plum_flower.title')}
      description={t('divination.plum_flower.description')}
      icon={CONFIG.icon}
      divinationType="plum_flower"
    >
      <div className="max-w-2xl mx-auto">
        <div className="space-y-4">
          <h4 className="font-medium">{t('divination.plum_flower.inputInstruction')}</h4>
          <div>
            <Label>{t('divination.plum_flower.number1')}</Label>
            <Input
              type="number"
              min={0}
              max={1000}
          ...
          <div>
            <Label>{t('divination.plum_flower.number2')}</Label>
            <Input
              type="number"
              min={0}
              max={1000}
        ...
        <div className="flex gap-2 md:gap-3 justify-center pt-4 md:pt-6">
          ...
          >
            ...
            {t('divination.plum_flower.viewResult')}
          </Button>
          <Button
          ...
          >
            {loading ? (
              <>
                ...
                {t('divination.plum_flower.divining')}
              </>
            ) : (
              <>
                <Sparkles className="h-4 w-4" />
                {t('divination.plum_flower.start')}
              </>
            )}
          </Button>
        </div>

...
export default function LoginPage() {
  ...
  const handleGithubCallback = async (code: string) => {
    ...
    try {
      ...
      console.error(err)
      setError(t('login.failed', { error: err.message || 'Unknown error' }))
    } finally {
      setLoading(false)
    }
  }
  ...
  const onGithubLogin = async () => {
    try {
      ...
      console.error(err)
      setError(t('login.failed', { error: err.message || '' }))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="flex justify-center items-center min-h-[60vh] px-4 animate-in fade-in duration-500">
      <Card className="w-full max-w-md backdrop-blur-lg bg-card/50 shadow-xl border-primary/10">
        <CardHeader className="space-y-1 md:space-y-2 text-center p-4 md:p-6">
          ...
          <CardDescription className="text-xs md:text-sm">{t('login.desc')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 p-4 md:p-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              <p className="text-sm font-medium text-muted-foreground">{t('login.logging')}</p>
            </div>
          )}

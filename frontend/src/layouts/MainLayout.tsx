                        ...
                        >
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-1 md:gap-2">
                                    <Button variant="ghost" size="icon" onClick={() => navigate('/')} title={t('layout.home')}>
                                    ...
                                    {settings.enable_login && (
                                        settings.user_name ? (
                                            ...
                                            <Button variant="default" size="sm" onClick={() => navigate('/login')} className="gap-2 bg-primary/80 hover:bg-primary">
                                                ...
                                                <span className="hidden sm:inline">{t('layout.login')}</span>
                                            </Button>
                                        )
                                    )}

                                    <Button variant="ghost" size="icon" onClick={() => navigate('/settings')} title={t('layout.settings')}>
                                        <Settings className="h-5 w-5" />
                                    </Button>

                                    <Button variant="ghost" size="icon" onClick={toggleDark} title={t('layout.toggleTheme')}>
                                            ...
                                            <div className="mt-2 pt-2 border-t border-border/50">
                                                <div className="flex items-center justify-between text-xs text-amber-500 bg-amber-500/10 px-3 py-1.5 rounded-md">
                                                    <span className="flex items-center gap-2">
                                                        ...
                                                        {t('layout.visitorRateLimit', { rate: settings.rate_limit })}
                                                    </span>
                                                    <Link to="/settings" className="hover:underline opacity-80 hover:opacity-100">
                                                        {t('layout.customizeConfig')}
                                                    </Link>
                                                </div>
                                            </div>
                                        )}
